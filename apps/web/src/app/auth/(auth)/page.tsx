...
const AuthPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // Handle register
        result = await registerMutation.mutateAsync({ name, email, password });

      }

      // Use NextAuth signIn with the received token
      ...
      if (signInResult?.error) {
        ...
        // Redirect to the board page using the userId from tRPC response
        router.push(`/board/`);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-[200px] flex items-center justify-center bg-gray-900 py-12 px-4 sm:px-6 lg:px-8 rounded-3xl">
