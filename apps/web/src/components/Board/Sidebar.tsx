...
const Sidebar: React.FC = () => {
  ...
  const handleDeleteBoard = async (boardId: string) => {
    try {
      ...
      if (selectedBoardName === boardsQuery.data?.find(b => b.id === boardId)?.name) {
  ...
  {boardsQuery.isLoading ? (
    ...
    boardsQuery.data?.map((board: Board) => (
      <div key={board.id} className="group relative flex items-center px-2 py-2 text-base font-medium rounded-md text-gray-300 hover:bg-gray-700 hover:text-white">
        {editingBoardId === board.id ? (
          ...
          <>
            <span onClick={() => handleBoardSelect(board)} className="flex-grow cursor-pointer">
              {board.name}
            </span>
            <div className="hidden group-hover:flex items-center">
              <button onClick={() => handleEditBoard(board)} className="ml-2 text-blue-500 hover:text-blue-400">
                <Edit size={20} />
              </button>
              {deletingBoardId === board.id ? (
                <button onClick={() => handleDeleteBoard(board.id)} className="ml-2 text-gray-400 hover:text-gray-200">
                  <Check size={20} />
                </button>
              ) : (
                <button onClick={() => setDeletingBoardId(board.id)} className="ml-2 text-red-500 hover:text-red-400">
                  <Trash size={20} />
                </button>
              )}
            </div>
